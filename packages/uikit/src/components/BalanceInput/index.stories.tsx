...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"BASE" | "USD">("BASE");
  const conversionUnit = editingUnit === "BASE" ? "USD" : "BASE";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "BASE" ? "USD" : "BASE";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "BASE") {
      ...
      setValues({
        BASE: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
