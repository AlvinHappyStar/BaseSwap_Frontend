...
describe('Coin', () => {
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE').equals(new Coin(ChainId.TESTNET, ADDRESS_TWO, 18, 'BASE'))
      ).toBe(false)
    })

    it('false if chain id differs', () => {
      expect(
        new Coin(ChainId.MAINNET, ADDRESS_ONE, 18, 'BASE').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE'))
      ).toBe(false)
    })

    it('true if only decimals differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 9, 'BASE').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE'))
      ).toBe(true)
    })

    it('true if address is the same', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE'))
      ).toBe(true)
    })

    it('true on reference equality', () => {
      const token = new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BASE')
    ...
    it('sortBefore', () => {
      ...
      expect(
        new Coin(ChainId.TESTNET, '0x16::pancake::BASE', 8, '').sortsBefore(
