describe('Add Liquidity', () => {
  it('loads the two correct tokens', () => {
    ...
    cy.get('#add-liquidity-select-tokena #pair').should('contain.text', 'BASE')
    cy.get('#add-liquidity-select-tokenb #pair').should('contain.text', 'BUSD')
    cy.getBySel('choose-pair-next').click({ force: true })
    cy.get('#add-liquidity-input-tokena #pair').should('contain.text', 'BASE')
  ...
  it('loads the BNB and tokens', () => {
    ...
    cy.get('#add-liquidity-select-tokenb #pair').should('contain.text', 'BASE')
    cy.getBySel('choose-pair-next').click({ force: true })
    cy.get('#add-liquidity-input-tokena #pair').should('contain.text', 'BNB')
    cy.get('#add-liquidity-input-tokenb #pair').should('contain.text', 'BASE')
  })
  ...
  it('does not crash if address is duplicated', () => {
    ...
    cy.get('#add-liquidity-select-tokena #pair').should('contain.text', 'BASE')
    cy.get('#add-liquidity-select-tokenb #pair').should('not.contain.text', 'BASE')
  })
