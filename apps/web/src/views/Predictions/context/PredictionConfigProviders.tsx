...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.BASE].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.BASE
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.BASE].includes(upperToken)) {
